...
class Dashboard:
    def __init__(self, root, processor):
        ...
        self.root.title("MediaMiner Studio - Professional Edition")
        # App icon (taskbar / title bar / Alt-Tab).
        try:
            _icon_dir = os.path.join(os.path.dirname(
                os.path.abspath(__file__)), "..", "assets")
            _ico = os.path.normpath(os.path.join(_icon_dir, "app_icon.ico"))
            if os.path.isfile(_ico):
                self.root.iconbitmap(_ico)
            else:
                _png = os.path.normpath(os.path.join(_icon_dir, "app_icon.png"))
