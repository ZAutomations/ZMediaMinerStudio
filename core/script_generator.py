...
{{"suggested_title": "your title here (max 12 words, natural, no em dashes or special chars)", "hashtag_1": "FirstHashtag", "hashtag_2": "SecondHashtag"}}"""
    ...
    def analyze_thumbnail(self, image_path, title="", niche="", language="",
        ...
        instruction = (
            ...
            "CRITICAL — POLICY-SAFE WORDING (the prompt you write is fed to an "
            "AI image generator that HARD-BLOCKS unsafe requests; if you use "
            "charged words the whole thumbnail fails to render). Therefore:\n"
            "• Convey drama ONLY through facial expression, lighting, color and "
            "composition — NEVER by depicting violence, physical harm, weapons, "
            "blood, injuries, bruises, restraints, or one person hurting another.\n"
            "• Replace any sensitive act with a neutral, newsroom-safe synonym: "
            "say 'heated family dispute' not 'abuse/beating', 'courtroom "
            "confrontation' or 'tense standoff' not 'attack', 'shocking "
            "accusation' or 'serious claim' not the specific crime, 'money "
            "disagreement' not 'theft/stealing', 'emotional confrontation' not "
            "'assault'. Keep it suggestive and dramatic, never graphic.\n"
            "• Emotions must stay non-violent: shocked, tearful, worried, "
            "pleading, defiant, disbelieving, pointing in argument — but NOT "
            "striking, grabbing, choking, or menacing anyone.\n"
            "• Overlay text must be a curiosity HOOK or QUESTION, never a "
            "defamatory statement of fact about a real person (write 'WHO IS "
            "LYING?' not 'SHE IS A THIEF'; 'THE SHOCKING TRUTH' not 'HE BEAT "
            "HER'). No slurs, no hate, no sexual content, no minors in distress.\n"
            "• Do not name real private individuals in the overlay text.\n"
            "The result should still look like a bold, high-CTR viral thumbnail "
            "— just described with clean, brand-safe language that will pass an "
            "automated image-safety filter on the first try.\n\n"
