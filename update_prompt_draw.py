...
6. VIDEO EDITING INSTRUCTIONS: Guide the tool on where to apply zoom, freeze frame, camera shake, draw circle, and draw arrow effects to enhance the storytelling.
...
[Play the original video with its actual audio. The viewer hears the real animal sounds. Mention key audio moments but do not describe every sound. Note: this section uses the raw footage audio, not narration.]
...
- Provide command type: ZOOM, FREEZE, SHAKE, DRAW_CIRCLE, DRAW_ARROW, or NONE
...
- DRAW_CIRCLE [center_x] [center_y] [radius] [duration_sec] [color] -- e.g., "DRAW_CIRCLE 400 300 50 3 RED" draws a red circle centered at (400,300) with radius 50 pixels for 3 seconds
- DRAW_ARROW [start_x] [start_y] [end_x] [end_y] [duration_sec] [color] -- e.g., "DRAW_ARROW 100 200 300 400 3 BLUE" draws an arrow from (100,200) to (300,400) in BLUE color for 3 seconds
...
3. CTA WORD COUNT:
   ...
   - CTA words are IN ADDITION TO the main narration word count (not included in it)
...
- DRAW_CIRCLE around the animal when first introducing/identifying it
- DRAW_ARROW pointing to the animal during key commentary moments
...
[0:05] | [DRAW_CIRCLE] | [DRAW_CIRCLE 400 300 50 3 RED] -- circle puppy at 0:05
[0:08] | [FREEZE] | [FREEZE 0:08 2] -- freeze on puppy reaction at first commentary spot
[0:08] | [DRAW_ARROW] | [DRAW_ARROW 350 250 450 350 3 BLUE] -- arrow pointing to puppy at 0:08
[0:12] | [SHAKE] | [SHAKE 8 1.5] -- mild shake transition
[0:20] | [FREEZE] | [FREEZE 0:20 2] -- freeze at second commentary spot
[0:20] | [DRAW_ARROW] | [DRAW_ARROW 350 250 450 350 3 BLUE] -- arrow pointing at 0:20
...
10. Available command types: ZOOM, FREEZE, SHAKE, DRAW_CIRCLE, DRAW_ARROW, NONE
...
- VIDEO EDITING INSTRUCTIONS with ZOOM/FREEZE/SHAKE/DRAW_CIRCLE/DRAW_ARROW commands timed to the video
...
ADD THESE DRAWING FEATURES:
- DRAW_CIRCLE: Draw a transparent red circle around the animal subject during freeze/zoom moments to highlight them. Format: "DRAW_CIRCLE 400 300 50 3 RED" (center_x, center_y, radius, duration_sec, color).
- DRAW_ARROW: Draw an arrow pointing to the animal subject during key commentary moments. Format: "DRAW_ARROW 100 200 300 400 3 BLUE" (start_x, start_y, end_x, end_y, duration_sec, color).

These drawing features should be used at the following timestamps:
- DRAW_CIRCLE at 0:05 (during hook, when puppy is first introduced)
- DRAW_ARROW at 0:08 (during first commentary spot, pointing to the puppy's reaction)
- DRAW_CIRCLE at 0:20 (during second commentary spot, highlighting the rooster)
- DRAW_ARROW at 0:25 (near CTA, pointing to the final moment)

The circle and arrow should use RED and BLUE colors respectively for maximum visibility against the animal footage, and should have 3-second duration to match the commentary spot timing.
'''
with codecs.open('data/script_prompts.json', 'w', encoding='utf-8') as f:
    json.dump(data, f, ensure_ascii=False, indent=2)
print('Updated narration_prompt successfully with DRAW_CIRCLE and DRAW_ARROW features')
